...
static void heteroTAM (double *Rone, double *Rtwo, double *dRone, double *dRtwo, hetRates *hetR, double *hetDim, double *dhetDim, struct rates *tr, double *Li) {
    ...
    const double dR15 = hetR->xFwd15 * tr->gasCur * hetDim[0] - hetR->xRev15 * hetDim[2];
    const double dR16 = hetR->xFwd16 * tr->gasCur * hetDim[1] - hetR->xRev16 * hetDim[2];
    ...
    const double dR15i = hetR->xFwd15 * (*Li) * hetDim[3] / tr->internalV - hetR->xRev15 * hetDim[5];
    const double dR16i = hetR->xFwd16 * (*Li) * hetDim[4] / tr->internalV - hetR->xRev16 * hetDim[5];
    ...
    dhetDim[0] += dR13 + dR14 - dR15; // AMD1
    dhetDim[1] += dR11 + dR12 - dR16; // MAD1
    dhetDim[2] += dR7 + dR8 + dR9 + dR10 + dR15 + dR16; // AMD2
    
    dhetDim[3] += dR13i + dR14i - dR15i;
    dhetDim[4] += dR11i + dR12i - dR16i;
    dhetDim[5] += dR7i + dR8i + dR9i + dR10i + dR15i + dR16i;
...
int AXL_react(double, N_Vector xIn, N_Vector dxdtIn, void *user_data) {
    ...
    // AM, MT, AT
    heteroTAM (&x_d[0], &x_d[13], &dxdt_d[0], &dxdt_d[13], &r->hetR[0], &x_d[37], &dxdt_d[37], r, &x_d[12]);
    heteroTAM (&x_d[13], &x_d[25], &dxdt_d[13], &dxdt_d[25], &r->hetR[1], &x_d[43], &dxdt_d[43], r, &x_d[12]);
    heteroTAM (&x_d[0], &x_d[25], &dxdt_d[0], &dxdt_d[25], &r->hetR[2], &x_d[49], &dxdt_d[49], r, &x_d[12]);
...
struct rates Param(double *params) {
    ...
    for (size_t ii = 0; ii < 3; ii++) {
        ...
        out.hetR[ii].xFwd15 = fmax(out.TAMs[hetRi[ii][1]].Binding1,out.TAMs[hetRi[ii][0]].Binding2);
        out.hetR[ii].xFwd16 = fmax(out.TAMs[hetRi[ii][0]].Binding1,out.TAMs[hetRi[ii][1]].Binding2);
        out.hetR[ii].xRev15 = out.hetR[ii].xFwd13*out.hetR[ii].xFwd15*out.hetR[ii].xRev10*KD21/out.hetR[ii].xFwd10/out.hetR[ii].xRev13;
        out.hetR[ii].xRev16 = out.hetR[ii].xFwd12*out.hetR[ii].xFwd16*out.hetR[ii].xRev8*KD22/out.hetR[ii].xFwd8/out.hetR[ii].xRev12;
    }
